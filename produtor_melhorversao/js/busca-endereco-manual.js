// Correções para Google Maps e busca de endereço (Baseado em produtor_old/js/maps-fix.js)
...
// Função initMap corrigida - NECESSÁRIA para callback da API do Google Maps
...
// Função searchAddressManual - PRINCIPAL FUNÇÃO QUE ESTAVA FALTANDO
window.searchAddressManual = function() {
    ...
    // Buscar endereço com componente de país para melhor precisão
    window.geocoder.geocode({ 
        ...
        if (status === 'OK' && results[0]) {
            ...
            // Mostrar campos de endereço após busca bem-sucedida
...
function preencherCamposEndereco(place) {
    ...
    ['venueName', 'street', 'city', 'number', 'neighborhood', 'state', 'cep'].forEach(fieldId => {
    ...
    // Atualizar preview se disponível
    if (typeof updatePreview === 'function') {
        updatePreview();
    }
    
    // Salvar dados do wizard
    if (typeof window.salvarEtapaAtual === 'function') {
        window.salvarEtapaAtual();
    } else if (typeof window.saveWizardData === 'function') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addressInput) {
        ...
        // Buscar ao sair do campo (com delay para evitar muitas buscas)
        let searchTimeout;
        ...
        console.warn('⚠️ Campo addressSearch não encontrado');
    }
});

console.log('✅ Correções do Google Maps carregadas - searchAddressManual disponível');
