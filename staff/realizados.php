...
<body>
        ...
        <main class="content-area">
            <div class="checkins-container">
                <div class="checkins-header">
                    <h1>Checkins Realizados</h1>
                    
                    <div class="controls-row">
                        <div class="search-container">
                            <input type="text" id="searchInput" placeholder="Buscar por nome do participante..." 
                                   onkeyup="searchParticipantes()" class="search-input">
                ...
                <div class="table-container">
                    <table class="checkins-table" id="checkinsTable">
                        <thead>
                            <tr>
                                <th>Foto</th>
                                <th>Nome</th>
    ...
    <script>
        ...
        // Função para carregar checkins
        function loadCheckins(page = 1, search = '') {
            ...
            fetch(`realizados_data.php?page=${page}&search=${encodeURIComponent(search)}`)
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        data.checkins.forEach(checkin => {
                            const row = createTableRow(checkin);
                            tableBody.appendChild(row);
                        });
                        
                        updatePagination(data.pagination);
                    } else {
                        tableBody.innerHTML = '<tr><td colspan="8" class="no-data">Nenhum check-in encontrado</td></tr>';
                    }
                })
                .catch(error => {
                    console.error('Erro:', error);
                    tableBody.innerHTML = '<tr><td colspan="8" class="error">Erro ao carregar dados</td></tr>';
                })
                .finally(() => {
        ...
        function createTableRow(checkin) {
            const row = document.createElement('tr');
            
            row.innerHTML = `
                <td>
                    <img src="${checkin.thumb}" alt="Foto" class="participant-thumb" 
                         onerror="this.src='https://anysummit.com.br/staff/img/user.jpg'">
                </td>
                <td class="participant-name">${checkin.nome}</td>
                <td>${checkin.email}</td>
                <td>${checkin.celular || '-'}</td>
                <td><span class="ticket-type">${checkin.tipoingresso}</span></td>
                <td class="staff-name">${checkin.staff_nome}</td>
                <td class="checkin-count">
                    <span class="count-badge">${checkin.total_checkins}x</span>
                </td>
                <td class="checkin-date">${formatDateTime(checkin.datacheck)}</td>
            `;
        ...
        function searchParticipantes() {
            ...
            loadCheckins(1, searchTerm);
        }

        // Função para exportar Excel
        function exportarExcel() {
            const search = document.getElementById('searchInput').value.trim();
            const url = `realizados_export.php?search=${encodeURIComponent(search)}`;
            
            // Cria um link temporário para download
            const link = document.createElement('a');
            link.href = url;
            link.download = `checkins_realizados_${new Date().toISOString().split('T')[0]}.xlsx`;
            link.click();
        }
        // Toggle mobile menu
