...
if ($_POST) {
    ...
    $sql = "SELECT staffid, nome, email, senha, eventoid FROM staff WHERE email = ? LIMIT 1";
    ...
    if ($stmt) {
        ...
        if ($row = mysqli_fetch_assoc($result)) {
            ...
            if ($row['senha'] === $senha || $row['senha'] === $senha_hash) {
                ...
                $update_sql = "UPDATE staff SET ultimologin = NOW() WHERE staffid = ?";
                $update_stmt = mysqli_prepare($con, $update_sql);
                if ($update_stmt) {
                    mysqli_stmt_bind_param($update_stmt, "i", $row['staffid']);
                ...
                $_SESSION['staff_nome'] = $row['nome'];
                $_SESSION['staff_email'] = $row['email'];
                $_SESSION['eventoid'] = $row['eventoid'];
                
                // Define os cookies sem expiração
                setCookieForever('staff_logado', '1');
                setCookieForever('staffid', $row['staffid']);
                setCookieForever('staff_nome', $row['nome']);
                setCookieForever('staff_email', $row['email']);
                setCookieForever('eventoid', $row['eventoid']);
                
             
                
                // Redireciona para a página de check-in
                header("Location: /staff/checkin");
                exit();
