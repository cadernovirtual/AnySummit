...
try {
    ...
    if (!$stmt_check) {
        throw new Exception("Erro na preparação da query de verificação: " . mysqli_error($con));
    }
    ...
    // Bind parameters: string, string, string, string, string, int, string
    ...
    $response = [
        'success' => false,
        'error' => 'Erro fatal do PHP: ' . $e->getMessage()
    ];
}
...
if (!empty($unwanted_output)) {
    error_log("Output inesperado no credenciamento: " . $unwanted_output);
}
