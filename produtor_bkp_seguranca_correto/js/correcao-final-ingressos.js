...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function corrigirBotoesIngressos() {
        ...
        ticketItems.forEach((item, index) => {
            console.log(`🎫 Corrigindo botões do ingresso ${index + 1}`);
            
            // Buscar botões existentes
            let editButton = item.querySelector('.btn-edit-ticket, button[onclick*="editarIngresso"]');
            let deleteButton = item.querySelector('.btn-delete-ticket, button[onclick*="excluirIngresso"]');
            
            // Se não existem, criar
            if (!editButton || !deleteButton) {
                const actionsContainer = item.querySelector('.ticket-actions') || criarContainerAcoes(item);
                
                if (!editButton) {
                    editButton = document.createElement('button');
                    editButton.type = 'button';
                    editButton.className = 'btn-edit-ticket';
                    editButton.innerHTML = '✏️'; // Ícone de edição
