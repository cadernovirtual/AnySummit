<?php
// API para processar cadastro completo - VERSÃO CORRIGIDA
...
// Log inicial
error_log("=== PROCESSAR CADASTRO DEBUG ===");
...
if ($_SESSION['verified_email'] !== $input['email']) {
    error_log("ERRO: Email não corresponde. Verificado: {$_SESSION['verified_email']}, Cadastro: {$input['email']}");
    http_response_code(401);
    echo json_encode([
        'success' => false,
...
try {
    ...
    error_log("Contratante inserido com ID: $contratanteId");
    ...
    $stmt = mysqli_prepare($con, $sql);
    if (!$stmt) {
        // Se falhar, tentar com senha_hash
        $sql = "INSERT INTO usuarios (
    ...
    if (!mysqli_stmt_execute($stmt)) {
        throw new Exception('Erro ao inserir usuário: ' . mysqli_stmt_error($stmt));
    }
    
    $usuarioId = mysqli_insert_id($con);
    mysqli_stmt_close($stmt);
    
    error_log("Usuário inserido com ID: $usuarioId");
    ...
    error_log("Login configurado com sucesso");
