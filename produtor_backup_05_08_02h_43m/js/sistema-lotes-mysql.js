...
window.excluirLoteDataInterface = async function(loteId) {
    try {
        ...
        // CRÍTICO: Invalidar cache imediatamente
        ...
        // CRÍTICO: Recarregar e renderizar interface
        console.log('🎨 Recarregando lotes e atualizando interface...');
        
        // Chamar a função de renderização unificada
        if (typeof window.renderizarLotesUnificado === 'function') {
            await window.renderizarLotesUnificado();
            console.log('✅ Interface atualizada via renderizarLotesUnificado');
        } else {
            console.warn('⚠️ renderizarLotesUnificado não disponível, tentando alternativa...');
        ...
        console.log('✅ Exclusão, renomeação e atualização de interface concluídas');
        return true;
