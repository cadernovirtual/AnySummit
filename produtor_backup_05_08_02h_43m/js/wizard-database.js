...
(function() {
    ...
    function salvarEtapaComEventoId(etapa) {
        ...
        switch(etapa) {
            ...
            case 7:
                console.log('💼 Salvando dados do produtor (etapa 7)...');
                
                // CORREÇÃO: Usar select em vez de radio button
                const producerSelect = document.getElementById('producer');
                const producerValue = producerSelect?.value || 'current';
                
                console.log('👤 Produtor selecionado:', producerValue);
                
                formData.append('produtor_selecionado', producerValue === 'current' ? 'atual' : 'novo');
                
                // Se selecionou "novo produtor", coletar os campos
                if (producerValue === 'new') {
                    const nomeProdutor = document.getElementById('producerName')?.value || '';
                    const nomeExibicao = document.getElementById('displayName')?.value || '';
                    const descricaoProdutor = document.getElementById('producerDescription')?.value || '';
                    
                    formData.append('nome_produtor', nomeProdutor);
                    formData.append('nome_exibicao_produtor', nomeExibicao);
                    formData.append('descricao_produtor', descricaoProdutor);
                    
                    console.log('💼 Coletando dados do novo produtor:', {
                        nome: nomeProdutor,
                        exibicao: nomeExibicao,
                        descricao: descricaoProdutor
                    });
                } else {
                    // Se selecionou produtor atual, limpar campos
                    formData.append('nome_produtor', '');
                    formData.append('nome_exibicao_produtor', '');
                    formData.append('descricao_produtor', '');
                    console.log('👤 Usando produtor atual - campos limpos');
                }
                break;
