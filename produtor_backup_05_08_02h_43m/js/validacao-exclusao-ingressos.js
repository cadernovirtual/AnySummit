...
function interceptarBotoesExclusao() {
    ...
    setTimeout(() => {
        ...
        document.addEventListener('click', async function(event) {
            ...
            // Verificações mais específicas para botões de exclusão DE INGRESSOS APENAS
            const onclickStr = target.onclick?.toString() || target.getAttribute('onclick') || '';
            
            // SE É BOTÃO DE LOTE - NÃO INTERCEPTAR
            if (onclickStr.includes('excluirLote') || 
                onclickStr.includes('Lote') ||
                target.title?.toLowerCase().includes('lote')) {
                console.log('🏷️ Botão de LOTE detectado - não interceptando:', target);
                return; // Deixar funcionar normalmente
            }
            
            // Só interceptar botões de INGRESSO (removeTicket)
            const isTicketDeleteButton = 
            ...
            if (!isTicketDeleteButton) {
                return; // Não é botão de exclusão de ingresso, continuar normalmente
            }
            
            console.log('🗑️ Botão de exclusão DE INGRESSO detectado:', target);
            ...
            // Tentar encontrar ID do INGRESSO (não lote)
            let ticketId = null;
            ...
            while (elemento && !ticketId && tentativas < 10) {
                ...
                // Extrair APENAS do onclick de removeTicket
                if (!ticketId) {
                    const onclickStr = elemento.onclick?.toString() || elemento.getAttribute('onclick') || '';
                    // Buscar especificamente por removeTicket (não outras funções)
            ...
            if (!ticketId) {
                console.warn('⚠️ ID do INGRESSO não encontrado, permitindo execução normal');
                // Se não conseguir encontrar ID de ingresso, permitir execução normal
                return;
            }
