    ...
    setTimeout(() => {
        ...
        seletoresBotoes.forEach(seletor => {
            ...
            elementos.forEach(elemento => {
                // CORREÇÃO: NÃO interceptar botões de exclusão de lotes
                const onclickStr = elemento.onclick?.toString() || elemento.getAttribute('onclick') || '';
                if (onclickStr.includes('excluirLote') || 
                    onclickStr.includes('excluir') && onclickStr.includes('lote') ||
                    elemento.classList.contains('btn-excluir-lote') ||
                    elemento.closest('.btn-excluir-lote')) {
                    console.log('🏷️ Botão de LOTE ignorado - não interceptando:', elemento);
                    return; // Não interceptar botões de lote
                }
        ...
        console.log(`✅ ${seletoresBotoes.length} tipos de botões interceptados (excluindo lotes)`);
    }, 2000);
}
