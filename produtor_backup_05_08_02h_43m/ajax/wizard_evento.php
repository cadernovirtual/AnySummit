...
switch ($action) {
    ...
    case 'carregar_dados_edicao':
        carregarDadosParaEdicao($con, $usuario_id);
        break;
...
function excluirLote($con, $usuario_id) {
    ...
    $sql = "SELECT id, nome, tipo FROM lotes WHERE id = ? AND evento_id = ?";
    $stmt = mysqli_prepare($con, $sql);
    mysqli_stmt_bind_param($stmt, "ii", $lote_id, $evento_id);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    ...
    // VERIFICAÇÃO OBRIGATÓRIA: Verificar se há ingressos associados
    error_log("🔍 Verificando ingressos associados ao lote...");
    $sql_ingressos = "SELECT COUNT(*) as total FROM ingressos WHERE evento_id = ? AND lote_id = ?";
    $stmt_ingressos = mysqli_prepare($con, $sql_ingressos);
    mysqli_stmt_bind_param($stmt_ingressos, "ii", $evento_id, $lote_id);
    mysqli_stmt_execute($stmt_ingressos);
    $result_ingressos = mysqli_stmt_get_result($stmt_ingressos);
    $row_ingressos = mysqli_fetch_assoc($result_ingressos);
    
    if ($row_ingressos['total'] > 0) {
        error_log("❌ Exclusão bloqueada - há {$row_ingressos['total']} ingressos associados");
        echo json_encode(['erro' => 'Não é possível excluir este lote pois existem ingressos associados a ele.']);
        return;
    }
    
    error_log("✅ Nenhum ingresso associado - prosseguindo com exclusão");
    
    // Salvar o tipo do lote para renomeação posterior
    $tipo_lote = $lote_info['tipo'];
    ...
    if (mysqli_stmt_execute($stmt)) {
        ...
        if ($affected_rows > 0) {
            ...
            // RENOMEAÇÃO OBRIGATÓRIA: Renomear lotes do mesmo tipo
            error_log("🔄 Iniciando renomeação dos lotes do tipo: $tipo_lote");
            renomearLotesPorTipo($con, $evento_id, $tipo_lote);
            error_log("✅ Renomeação concluída");
...
function renomearLotesPorTipo($con, $evento_id, $tipo) {
    try {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            mysqli_stmt_bind_param($stmt_update, "si", $novo_nome, $row['id']);
...
function salvarEdicaoEvento($con, $usuario_id) {
    try {
        ...
        error_log("=== SALVANDO EDIÇÃO DO EVENTO ===");
        error_log("Evento ID: $evento_id");
        error_log("Usuario ID: $usuario_id");
        ...
        if (isset($dados['cor_fundo'])) {
            $campos[] = "cor_fundo_alternativa = ?";
            $valores[] = $dados['cor_fundo'];
            $tipos .= 's';
        }
        
        // Imagens (salvar caminhos se fornecidos)
        ...
        if (isset($dados['capa']) && !empty($dados['capa'])) {
            $campos[] = "capa_path = ?";
            $valores[] = $dados['capa'];
            $tipos .= 's';
        }
        
        if (isset($dados['fundo']) && !empty($dados['fundo'])) {
            $campos[] = "fundo_path = ?";
            $valores[] = $dados['fundo'];
            $tipos .= 's';
        }
        ...
        if (isset($dados['categoria_id'])) {
            $campos[] = "categoria_id = ?";
            $valores[] = intval($dados['categoria_id']);
            $tipos .= 'i';
        }
        ...
        if (isset($dados['tipo_local'])) {
            if ($dados['tipo_local'] === 'online') {
                $campos[] = "link_transmissao = ?";
                $valores[] = $dados['link_evento'] ?? '';
                $tipos .= 's';
                
                // Limpar dados presenciais
                $campos[] = "nome_local = NULL, cep = NULL, endereco = NULL, numero = NULL, complemento = NULL, bairro = NULL, cidade = NULL, estado = NULL, latitude = NULL, longitude = NULL";
            } else {
                // Evento presencial
                ...
                // Limpar link de transmissão
                $campos[] = "link_transmissao = NULL";
            }
        }
        
        // Atualizar timestamp de modificação
        $campos[] = "modificado_em = NOW()";
        ...
        // Construir e executar query de atualização
        $sql_update = "UPDATE eventos SET " . implode(', ', $campos) . " WHERE id = ? AND usuario_id = ?";
        $valores[] = $evento_id;
        $valores[] = $usuario_id;
        $tipos .= 'ii';
        ...
        $stmt_update = mysqli_prepare($con, $sql_update);
        if (!$stmt_update) {
            error_log("Erro ao preparar statement: " . mysqli_error($con));
            echo json_encode(['erro' => 'Erro interno do servidor']);
            return;
        }
        
        mysqli_stmt_bind_param($stmt_update, $tipos, ...$valores);
        
        if (mysqli_stmt_execute($stmt_update)) {
            $affected_rows = mysqli_stmt_affected_rows($stmt_update);
            error_log("✅ Evento atualizado com sucesso. Linhas afetadas: $affected_rows");
            
            echo json_encode([
                'sucesso' => true,
                'message' => 'Evento atualizado com sucesso',
                'evento_id' => $evento_id,
                'linhas_afetadas' => $affected_rows
            ]);
        } else {
            error_log("❌ Erro ao executar update: " . mysqli_stmt_error($stmt_update));
            echo json_encode(['erro' => 'Erro ao salvar alterações']);
        }
        
        mysqli_stmt_close($stmt_update);
        
    } catch (Exception $e) {
        error_log("❌ Erro ao salvar edição do evento: " . $e->getMessage());
...
/**
 * Carregar dados do evento para edição (função simplificada)
 */
function carregarDadosParaEdicao($con, $usuario_id) {
    ...
    try {
        ...
        error_log("=== CARREGAR DADOS PARA EDIÇÃO ===");
        error_log("Evento ID: $evento_id");
        error_log("Usuario ID: $usuario_id");
        ...
        if (!$result || mysqli_num_rows($result) == 0) {
            error_log("❌ Evento não encontrado para usuario $usuario_id");
            echo json_encode(['erro' => 'Evento não encontrado ou sem permissão']);
            exit;
        }
