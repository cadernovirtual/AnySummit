...
if (!$column_exists) {
    ...
    <head>
        <meta charset="UTF-8">
        <title>Configuração Necessária - AnySummit</title>
        <style>
            body { font-family: Arial, sans-serif; margin: 40px; background: #f5f5f5; }
            .alert { padding: 20px; background: #f8d7da; color: #721c24; border: 1px solid #f5c6cb; border-radius: 8px; }
            .btn { background: #007bff; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; display: inline-block; margin-top: 15px; }
        </style>
    </head>
    <body>
        <div class="alert">
            <h3>⚠️ Configuração Necessária</h3>
            <p>As alterações do banco de dados ainda não foram aplicadas.</p>
            <p>É necessário executar as alterações para adicionar as colunas <code>logomarca</code> e <code>usuario_id</code> na tabela <code>contratantes</code>.</p>
            <a href="executar_alteracoes_web.php" class="btn">🔧 Executar Alterações do Banco</a>
            <a href="meuseventos.php" class="btn">⬅️ Voltar aos Eventos</a>
        </div>
    </body>
    </html>
    <?php
        ...
        if (empty($nome_fantasia)) {
            ...
            if ($organizador_id > 0) {
                ...
                if (!empty($logomarca)) {
                    ...
                    mysqli_stmt_bind_param($stmt, "sssssssssssisii", 
                        $nome_fantasia, $razao_social, $cnpj, $cpf, $email_contato, 
                        $telefone, $endereco_completo, $tipo_recebimento, $chave_pix, 
