    ...
    let currentStep = 1;
        ...
        */
        ...
        function initAdditionalUploads() {
            console.log('🎨 Inicializando uploads adicionais...');
            
            // Logo do evento
            const logoUpload = document.getElementById('logoUpload');
            if (logoUpload) {
                console.log('✅ Input logoUpload encontrado');
                logoUpload.addEventListener('change', function() {
                    console.log('📷 Logo upload triggered');
                    handleImageUpload(this, 'logoPreviewContainer', 'logo');
                });
            } else {
                console.error('❌ Input logoUpload não encontrado');
            }

            // Capa quadrada
            const capaUpload = document.getElementById('capaUpload');
            if (capaUpload) {
                console.log('✅ Input capaUpload encontrado');
                capaUpload.addEventListener('change', function() {
                    console.log('📷 Capa upload triggered');
                    handleImageUpload(this, 'capaPreviewContainer', 'capa');
                });
            } else {
                console.error('❌ Input capaUpload não encontrado');
            }
        ...
        async function handleImageUpload(input, containerId, type) {
            ...
            reader.onload = function(e) {
                console.log('📸 Preview carregado para', type);
                const container = document.getElementById(containerId);
                console.log('📦 Container encontrado:', !!container, containerId);
                ...
                if (container) {
                    ...
                    `;
                    console.log('✅ HTML atualizado para', type);
        ...
        function mostrarSucesso(dados) {
            ...
            fetch('/produtor/ajax/limpar_wizard.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
            }).then(() => {
                console.log('✅ Wizard limpo do banco de dados');
            }).catch(error => {
                console.error('Erro ao limpar wizard do banco:', error);
            });
            
            // Redirecionar para página de sucesso
            console.log('✅ Redirecionando para página de sucesso...');
            window.location.href = `/produtor/evento-publicado.php?eventoid=${eventoId}`;
        }
        ...
        function addTicketToList(type, title, quantity, price, loteId = '', description = '', saleStart = '', saleEnd = '', minQuantity = 1, maxQuantity = 5) {
            ...
            const uniqueTicketId = `ticket_${Date.now()}_${ticketCount}`;

            const ticketItem = document.createElement('div');
            ticketItem.className = 'ticket-item';
            ticketItem.dataset.ticketId = uniqueTicketId;
