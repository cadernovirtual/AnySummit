...
// Verificar se deve retomar wizard
...
if ($retomar_wizard) {
    try {
        $sql = "SELECT dados_wizard, step_atual 
        ...
        if ($stmt) {
            ...
            if ($row = mysqli_fetch_assoc($result)) {
                ...
                $wizard_step = $row['step_atual'];
            }
            mysqli_stmt_close($stmt);
        }
    } catch (Exception $e) {
        error_log("Erro ao recuperar wizard: " . $e->getMessage());
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/qr-scanner/1.4.2/qr-scanner.umd.min.js"></script>
      ...
      <link rel="stylesheet" type="text/css" href="/produtor/css/checkin-painel-1-0-0.css">
        ...
        <link rel="stylesheet" type="text/css" href="/produtor/css/conditional-fix.css" />
...
<body>
    ...
    window.usuarioId = <?php echo $_SESSION['usuarioid'] ?? 'null'; ?>;
    window.usuarioNome = '<?php echo addslashes($_SESSION['usuario_nome'] ?? ''); ?>';
    window.contratanteId = <?php echo $_SESSION['contratanteid'] ?? 'null'; ?>;
    </script>
                    ...
                    <div class="form-grid">
                        <div class="form-group">
                            ...
                            <select id="producer">
                                ...
                                $nome_contratante = "Produtor Atual";
                                if (isset($_SESSION['contratanteid'])) {
                                    $query = mysqli_query($con, "SELECT nome_fantasia FROM contratantes WHERE id = " . $_SESSION['contratanteid']);
                                    if ($row = mysqli_fetch_assoc($query)) {
                                        $nome_contratante = $row['nome_fantasia'];
                                    }
                                }
                                ?>
                                <option value="current">Você (<?php echo htmlspecialchars($nome_contratante); ?>)</option>
    ...
    <!-- ==================== FIM DOS MODAIS DE LOTES ==================== -->
     ...
     <script language='javascript' src="/produtor/js/api-config-dev.js?v=<?php echo time(); ?>"></script>
     ...
     <script src="js/api-config-dev.js?v=<?php echo time(); ?>"></script>
     ...
     <!-- REMOVENDO TODOS OS FIXES DESNECESSÁRIOS QUE ESTAVAM QUEBRANDO TUDO
...
<script src="js/color-picker-fix.js?v=<?php echo time(); ?>"></script>
<script src="js/combo-save-fix.js?v=<?php echo time(); ?>"></script>
<script src="js/combo-items-fix.js?v=<?php echo time(); ?>"></script>
<script src="js/combo-fix-robusto.js?v=<?php echo time(); ?>"></script>
<script src="js/upload-images-fix.js?v=<?php echo time(); ?>"></script>
<script src="js/fix-separacao-combos.js?v=<?php echo time(); ?>"></script>
<script src="js/fix-ids-consistencia.js?v=<?php echo time(); ?>"></script>
<script src="js/fix-recupera-items-combo.js?v=<?php echo time(); ?>"></script>
<script src="js/fix-datas-lote.js?v=<?php echo time(); ?>"></script>
<script src="js/limpeza-ultra-agressiva.js?v=<?php echo time(); ?>"></script>
<script src="js/fix-recuperacao-dialog.js?v=<?php echo time(); ?>"></script>
...
<script src="js/combo-trash-icon-fix.js?v=<?php echo time(); ?>"></script>
     -->
<!-- Arquivos de fixes comentados temporariamente para debug
...
<!-- Reativando apenas os scripts essenciais para upload e preview -->
<script src="js/upload-images-fix.js?v=<?php echo time(); ?>"></script>
<script src="js/preview-fix.js?v=<?php echo time(); ?>"></script>
<script src="js/complete-fixes.js?v=<?php echo time(); ?>"></script>

<!-- Script de debug temporário removido -->
<!-- <script src="js/debug-upload.js?v=<?php echo time(); ?>"></script> -->

<?php if ($retomar_wizard && $wizard_data): ?>
<script>
// Retomar wizard abandonado
document.addEventListener('DOMContentLoaded', function() {
    console.log('🔄 Retomando wizard da etapa <?php echo $wizard_step; ?>');
    
    // Carregar dados salvos
    const wizardData = <?php echo $wizard_data; ?>;
    
    // Aguardar um momento para garantir que tudo foi inicializado
    setTimeout(function() {
