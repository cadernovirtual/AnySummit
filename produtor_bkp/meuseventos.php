...
// Não vamos mais usar funções de helpers, vamos forçar o caminho diretamente
...
// Verificar se existe wizard abandonado
...
try {
    ...
    $stmt_wizard = mysqli_prepare($con, $sql_wizard);
    if ($stmt_wizard) {
        mysqli_stmt_bind_param($stmt_wizard, "ii", $usuario_id, $contratante_id);
        mysqli_stmt_execute($stmt_wizard);
        $result_wizard = mysqli_stmt_get_result($stmt_wizard);
        
        if ($row_wizard = mysqli_fetch_assoc($result_wizard)) {
            // Verificar se o wizard tem dados e não está completo
            $dados = json_decode($row_wizard['dados_wizard'], true);
            if (!empty($dados) && $row_wizard['step_atual'] < 8) {
                $wizard_abandonado = true;
                $wizard_step = $row_wizard['step_atual'];
                $wizard_atualizado = $row_wizard['atualizado_em'];
            }
        }
        mysqli_stmt_close($stmt_wizard);
    }
} catch (Exception $e) {
    error_log("Erro ao verificar wizard abandonado: " . $e->getMessage());
}
...
<body>
    ...
    <div class="main-layout">
        ...
        <main class="content-area">
            <div class="eventos-container">
                ...
                <div class="wizard-alert" style="background: #fff3cd; border: 1px solid #ffeaa7; border-radius: 8px; padding: 15px; margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between;">
                    <div>
                        <strong>🔔 Você tem um evento não finalizado!</strong>
                        <p style="margin: 5px 0 0 0; color: #666;">Você parou na etapa <?php echo $wizard_step; ?> de 8. Deseja continuar de onde parou?</p>
                    </div>
                    <div style="display: flex; gap: 10px;">
                        <a href="/produtor/novoevento?retomar=1" class="btn" style="background: #28a745; color: white; padding: 8px 20px; border-radius: 4px; text-decoration: none;">
                            Continuar
                        </a>
                        <button onclick="descartarWizard()" class="btn" style="background: #dc3545; color: white; padding: 8px 20px; border-radius: 4px; border: none; cursor: pointer;">
                            Descartar
                        </button>
                    </div>
                </div>
                                    ...
                                    <td>
                                        <div style="display: flex; align-items: center; gap: 12px;">
                                            ...
                                            if (!empty($evento['imagem_capa'])) {
                                                // Forçar o caminho correto /uploads/eventos/
                                                $nome_arquivo = basename($evento['imagem_capa']);
                                                $caminho_correto = '/uploads/eventos/' . $nome_arquivo;
                                                echo '<img src="' . htmlspecialchars($caminho_correto) . '" alt="Capa" class="table-image">';
                                            } else {
                                    ...
                                    <td>
                                        <div class="actions-dropdown">
                                            ...
                                            <div class="dropdown-content">
                                                <a href="/produtor/editar-evento/<?php echo $evento['id']; ?>" class="dropdown-item">
                                                    ✏️ Editar
                                                </a>
                        ...
                        <div class="evento-card-mobile">
                            <div class="evento-card-header">
                                ...
                                if (!empty($evento['imagem_capa'])) {
                                    ...
                                    $nome_arquivo = basename($evento['imagem_capa']);
                                    $caminho_correto = '/uploads/eventos/' . $nome_arquivo;
                                    echo '<img src="' . htmlspecialchars($caminho_correto) . '" alt="Capa" class="evento-card-image">';
                                } else {
                                    echo '<div class="evento-card-image">🖼️</div>';
                                }
                                ?>
    ...
    // Função para descartar wizard
    function descartarWizard() {
        if (confirm('Tem certeza que deseja descartar o evento não finalizado?')) {
            fetch('/produtor/ajax/descartar_wizard.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    alert('Erro ao descartar wizard');
                }
            })
            .catch(error => {
