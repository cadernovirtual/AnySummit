...
try {
    ...
    // ========================================
    // ENVIAR EMAIL DE CONFIRMAÇÃO PARA PAGAMENTOS GRATUITOS
    // ========================================
    if ($metodo_pagamento === 'gratuito') {
        error_log("Enviando email de confirmação para pagamento gratuito - Pedido: $pedidoid");
            ...
            if (function_exists('enviarEmailConfirmacao')) {
                try {
                    ...
                    if ($email_enviado === true) {
                        error_log("Email de confirmação enviado com sucesso para pedido gratuito: $pedidoid");
                    } else {
                        error_log("Falha ao enviar email de confirmação para pedido gratuito: $pedidoid - Erro: " . $email_enviado);
                    }
                } catch (Exception $e) {
                    error_log("Exceção ao enviar email de confirmação gratuito: " . $e->getMessage());
                }
            }
        }
            ...
            if (function_exists('notificarOrganizadorCompra')) {
                try {
                    ...
                    if ($notificacao_enviada === true) {
                        error_log("Notificação do organizador enviada com sucesso para pedido gratuito: $pedidoid");
                    } else {
                        error_log("Falha ao enviar notificação do organizador para pedido gratuito: $pedidoid - Erro: " . $notificacao_enviada);
                    }
                } catch (Exception $e) {
                    error_log("Exceção ao enviar notificação do organizador gratuito: " . $e->getMessage());
                }
            }
        }
    }
