...
try {
    ...
    $webhook_data = [
        ...
        'ingresso' => [
            ...
            // CORREÇÃO: Adicionar link de validação para o destinatário acessar
        ...
        // CORREÇÃO: Adicionar instruções claras sobre o que deve ser enviado
        'instrucoes_email' => [
            'assunto' => 'Você recebeu um ingresso para ' . ($evento_dados['nome'] ?? 'evento'),
            'tipo_envio' => 'link_para_vinculacao', // Não é o ingresso final, apenas link
            'acao_necessaria' => 'destinatario_deve_vincular_dados'
        ]
    ];
    ...
    if (!$webhook_response['success']) {
        ...
        // CORREÇÃO: Enviar email diretamente como backup quando webhook falha
        try {
            error_log('Tentando envio direto como backup...');
            ...
            curl_setopt($ch_backup, CURLOPT_POST, 1);
            curl_setopt($ch_backup, CURLOPT_POSTFIELDS, json_encode($backup_data));
            curl_setopt($ch_backup, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
            curl_setopt($ch_backup, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch_backup, CURLOPT_TIMEOUT, 15);
            curl_setopt($ch_backup, CURLOPT_SSL_VERIFYPEER, false);
            
            $backup_result = curl_exec($ch_backup);
            $backup_http_code = curl_getinfo($ch_backup, CURLINFO_HTTP_CODE);
            curl_close($ch_backup);
            
            if ($backup_result && $backup_http_code == 200) {
                $backup_response = json_decode($backup_result, true);
                if ($backup_response && $backup_response['success']) {
                    error_log('Email enviado com sucesso via backup direto');
                    $webhook_response['backup_email_sent'] = true;
                } else {
                    error_log('Falha no backup de email: ' . ($backup_response['message'] ?? 'Erro desconhecido'));
                }
            } else {
                error_log('Falha na comunicação com API backup de email');
            }
            
        } catch (Exception $backup_e) {
            error_log('Erro no backup de email: ' . $backup_e->getMessage());
        }
    }
    
    // CORREÇÃO: NÃO marcar automaticamente como transferido
    // O ingresso só deve ser marcado como vinculado quando o destinatário acessar e preencher seus dados
    // Por enquanto, apenas registrar que foi enviado mas manter status ativo
    // Usaremos transferido_para_email temporariamente para rastrear a quem foi enviado
    // mas sem alterar o status (que continuará 'ativo')
    ...
    echo json_encode([
        ...
        'dados' => [
            ...
            'backup_email_enviado' => $webhook_response['backup_email_sent'] ?? false,
            'metodo_envio' => ($webhook_response['success'] ?? false) ? 'webhook' : 'backup_direto',
            'link_validacao' => 'https://anysummit.com.br/validar-ingresso.php?h=' . $ingresso_db['hash_validacao'],
