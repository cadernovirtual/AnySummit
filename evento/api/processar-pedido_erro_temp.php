...
try {
    ...
    // ========================================
    // 6. ENVIAR EMAIL DE CONFIRMAÇÃO PARA PAGAMENTOS GRATUITOS (SEM BLOQUEAR)
    // ========================================
    if ($metodo_pagamento === 'gratuito') {
        // Execução assíncrona do envio de email para não bloquear a resposta
        try {
            error_log("Tentando enviar email de confirmação para pagamento gratuito - Pedido: $pedidoid");
            
            // Incluir função de envio de email se existir
            $email_file = __DIR__ . "/enviar-email-confirmacao.php";
            if (file_exists($email_file)) {
                include_once($email_file);
                
                if (function_exists('enviarEmailConfirmacao')) {
                    $email_enviado = enviarEmailConfirmacao($pedidoid, $con);
                    error_log("Resultado email confirmacao gratuito: " . ($email_enviado === true ? "sucesso" : $email_enviado));
                }
            }
            
            // Incluir notificação do organizador se existir
            $notificar_file = __DIR__ . "/notificar-organizador.php";
            if (file_exists($notificar_file)) {
                include_once($notificar_file);
                
                if (function_exists('notificarOrganizadorCompra')) {
                    $notificacao_enviada = notificarOrganizadorCompra($pedidoid, $con);
                    error_log("Resultado notificacao organizador gratuito: " . ($notificacao_enviada === true ? "sucesso" : $notificacao_enviada));
                }
            }
        } catch (Exception $email_error) {
            // Log do erro mas não quebrar o fluxo
            error_log("Erro no envio de email gratuito (não crítico): " . $email_error->getMessage());
        }
    } em: " . $notificar_file);
        }
    }
