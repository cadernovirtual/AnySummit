...
function excluirLoteData(id) {
    ...
    if (verificarIngressosNoLote(id)) {
...
// Função para verificar se há ingressos associados ao lote
function verificarIngressosNoLote(loteId) {
    // Verificar nos ingressos salvos
    const ingressos = window.ingressos || [];
    const ingressosData = window.ingressosData || [];
    const temporaryTickets = window.temporaryTickets || [];
    
    // Combinar todos os ingressos
    const todosIngressos = [...ingressos, ...ingressosData];
    
    // Verificar se algum ingresso está associado a este lote
    return todosIngressos.some(ingresso => {
        return ingresso.lote_id === loteId || 
               ingresso.loteId === loteId ||
               ingresso.lote === loteId;
    });
}
