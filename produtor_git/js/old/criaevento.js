...
(function() {
    ...
    // Usar variável global para currentStep
    ...
    let currentStep = window.currentStep;
        ...
        function updateStepDisplay() {
            const stepAtual = window.currentStep;
            ...
            const progressPercentage = ((stepAtual - 1) / (totalSteps - 1)) * 100;
        ...
        function validateStep(stepNumber) {
            ...
            switch(stepNumber) {
                case 1:
                    ...
                    if (!hasLogo) {
                        isValid = false;
                        if (logoContainer) logoContainer.classList.add('error-field');
                    }
                    
                    if (!hasCapa) {
                        isValid = false;
                        if (capaContainer) capaContainer.classList.add('error-field');
                    }
                ...
                case 5:
                    ...
                    isValid = hasLotesDOM || hasLotesArray;
                    break;
                    
                case 6:
                    // Validar se há pelo menos um ingresso
                    const ticketCards = document.querySelectorAll('.ticket-card');
                    const hasIngressosDOM = ticketCards && ticketCards.length > 0;
                    const hasIngressosArray = (window.ingressos && window.ingressos.length > 0) || 
                                            (window.ingressosData && window.ingressosData.length > 0);
                    
                    isValid = hasIngressosDOM || hasIngressosArray;
                    break;
                    
                case 7:
                    // Etapa 7 sempre válida (produtor)
                    isValid = true;
                    break;
            ...
            // Mostrar mensagem se não for válido
            if (!isValid) {
        ...
        */
        ...
        function mostrarSucesso(dados) {
            ...
            // Limpar todos os dados do wizard antes de redirecionar
            clearAllWizardData();
...
window.getCurrentStep = function() { return window.currentStep; };
...
window.nextStep = function() {
    ...
    if (validateStep(window.currentStep)) {
        if (window.currentStep < totalSteps) {
            ...
            window.wizardState.currentStep = window.currentStep;
            updateStepDisplay();
...
window.verificarIngressoEmCombo = function(ticketId) {
    const combos = window.combos || [];
    const comboItems = window.comboItems || [];
    
    // Verificar se o ingresso está em algum combo
    return combos.some(combo => {
        return combo.items && combo.items.some(item => 
